...
import { z } from "zod";
import useCadastrarParticipacao from "../hooks/useCadastrarParticipacao";
...
const schema = z.object({
  ...
  usuario: z
    .string().nonempty({ message: "O usuario deve ser informado." }),
  data_cadastro: z
    .string()
    .min(1, { message: "A data de cadastro deve ser informada." })
    .regex(regexData, { message: "Data inválida." }),
  xp: z
    .number({ invalid_type_error: "O xp deve ser informado. " })
    .min(1, { message: "O xp mínimo é 1." }),
  credito: z
      .number({ invalid_type_error: "O credito deve ser informado. " })
      .min(1, { message: "O credito mínimo é 5." }),
  imagem: z
    .string()
...
const CadastroDeParticipacoesForm = () => {
  ...
  //   const pessoa = {nome: "Joao", endereco: "Rua X número 10"};
  //   const func = {...pessoa, cargo: "Programador"}
  const cadastrarParticipacao = useCadastrarParticipacao();
  ...
  const onSubmit = ({
    ...
    cadastrarParticipacao.mutate(participacao);
  };

  if (cadastrarParticipacao.error) throw cadastrarParticipacao.error;
  if (errorUsuario) throw errorUsuario; // Para que a página de erro
  // seja exibida.
...
export default CadastroDeParticipacoesForm;
